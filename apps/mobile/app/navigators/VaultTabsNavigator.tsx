...
const INDICATOR_ANIMATION_DURATION = 90;
const TAB_BAR_HIDE_ANIMATION_DURATION = 110;

type VaultTabScreenOptions = BottomTabNavigationOptions & {
  unmountOnBlur: false;
};
...
function logTabs(message: string, meta?: Record<string, unknown>) {
  if (!__DEV__) return;
  console.log(`[tabs] ${message}`, meta ?? "");
}
...
function FloatingTabBar({ state, descriptors, navigation }: BottomTabBarProps) {
  ...
  const lastPressRef = useRef<{ name: string; time: number } | null>(null);

  const shouldHide = useMemo(
    () =>
      state.routes.some((route, index) => {
  ...
  useEffect(() => {
    const routeName = state.routes[state.index]?.name ?? "unknown";
    const time = nowMs();
    const lastPress = lastPressRef.current;
    logTabs(`state ${routeName} at ${time.toFixed(1)}`, {
      deltaFromPress:
        lastPress && lastPress.name === routeName
          ? `${(time - lastPress.time).toFixed(1)}ms`
          : undefined,
    });
  }, [state.index, state.routes]);
...
function VaultTabIcon({
  ...
  if (icon === "security") {
    return (
      <Shield size={20} fill={focused ? color : undefined} color={color} />
    );
  }
...
function shouldHideTabBar(
  ...
  if (route.name === "Vault")
    return hiddenVaultScreens.includes(focused as keyof VaultStackParamList);
  if (route.name === "Security") {
    return hiddenSecurityScreens.includes(
      focused as keyof SecurityStackParamList,
    );
  }
  if (route.name === "Settings") {
    return hiddenSettingsScreens.includes(
      focused as keyof SettingsStackParamList,
    );
  }
  return false;
}
