...
import {
  getPrivacyPrefs,
  setPrivacyPrefs,
} from "@/locker/security/privacyPrefsRepo";
import {
...
function logSecurity(message: string, meta?: Record<string, unknown>) {
  if (!__DEV__) return;
  console.log(`[SecurityDashboard] ${message}`, meta ?? "");
}
...
const SecurityDashboardScreenComponent: FC<
  SecurityStackScreenProps<"SecurityDashboard">
> = function SecurityDashboardScreenComponent(props) {
  ...
  if (__DEV__) {
    ...
    console.log("[Security] render", renderCountRef.current);
  }
  ...
  const [privacyPrefs, setLocalPrivacyPrefs] = useState(() =>
    getPrivacyPrefs(),
  );
  ...
  const shouldAnimateIntro = useSessionIntroAnimation(
    "security-dashboard-intro",
    !reducedMotion,
  );

  const refreshStatus = useCallback(async () => {
    const start = nowMs();
    logSecurity("status load start");
    try {
    ...
    logSecurity("status load end", {
      durationMs: (nowMs() - start).toFixed(1),
    });
  }, []);

  useEffect(() => {
    logSecurity("mounted", { time: nowMs().toFixed(1) });
  }, []);

  useFocusEffect(
    useCallback(() => {
      const focusTime = nowMs();
      if (__DEV__) console.log("[Security] focus");
      logSecurity("focus", { time: focusTime.toFixed(1) });
      const task = InteractionManager.runAfterInteractions(() => {
        logSecurity("deferred focus work start", {
  ...
  const handleSetBackup = useCallback(async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to save backup";
      setError(message);
    }
  ...
  const handleDeleteBackup = useCallback(async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to remove backup";
      setError(message);
    }
  }, [refreshStatus]);
  ...
  const overviewWarnings = useMemo(() => {
    ...
    if (!recovery.configured)
      warnings.push("Recovery backup is not configured.");
    warnings.push(...tamperIndicators);
    return warnings;
  }, [passkeyReady, recovery.configured, tamperIndicators]);

  const heroChips = useMemo<
    Array<{ label: string; tone: "accent" | "warning" | "neutral" }>
  >(
    () => [
      {
        label: `Trust ${trust.state}`,
  ...
  function SummaryStatV2({
    ...
    return (
      <View
        style={themed([$summaryStatCardV2, accent && $summaryStatCardAccentV2])}
      >
        <Text style={themed($summaryStatLabelV2)}>{label}</Text>
        <Text
          style={themed(
            accent ? $summaryStatValueAccentV2 : $summaryStatValueV2,
          )}
        >
          {value}
        </Text>
  ...
  const activitySummary = useMemo(() => {
    ...
    activityEvents.forEach((event) => {
      ...
      if (event.type === "step_up_success" || event.type === "trust_cleared")
        counts.elevated += 1;
  ...
  const activityGrid = useMemo(
    () => buildActivityGrid(activityEvents),
    [activityEvents],
  );
  ...
  return (
    <Screen
      ...
      contentContainerStyle={themed([
        $content,
        $insets,
        { paddingBottom: 105 + safeInsets.bottom },
      ])}
        ...
        >
          ...
          >
            <View style={themed($activityChartShellV2)}>
              <Text style={themed($activityHintV2)}>
                Last recorded audit windows
              </Text>
      ...
      <View>
        ...
        <Text style={themed($activityHintV2)}>Last recorded audit windows</Text>
              ...
              <View style={themed($activitySummaryRowV2)}>
                <SummaryStatV2
                  label="Unlocks"
                  value={String(activitySummary.unlocks)}
                />
                <SummaryStatV2
                  label="Auto-locks"
                  value={String(activitySummary.autoLocks)}
                />
                <SummaryStatV2
                  label="Step-up / Trust"
                  value={String(activitySummary.elevated)}
                />
                <SummaryStatV2
                  label="Alerts"
                  value={String(activitySummary.alerts)}
                  accent
                />
              </View>
        ...
        <View style={themed($stack)}>
          ...
          >
            <View style={themed($toggleCardGrid)}>
              <CompactToggleCard
                ...
                onPress={() =>
                  handlePrivacyToggle({
                    lockOnBackground: !privacyPrefs.lockOnBackground,
                  })
                }
              />
          ...
          <AccordionSection
            ...
            rightValue={
              overviewWarnings.length > 0
                ? `${overviewWarnings.length} warnings`
                : "Healthy"
            }
          ...
          >
            <View style={themed($overviewGrid)}>
              <CompactInfoPill
                label="Passkey"
                value={passkeyReady ? "Enabled" : "Off"}
              />
              <CompactInfoPill
                label="Recovery"
                value={recovery.configured ? "Ready" : "Missing"}
              ...
              <CompactInfoPill
                label="Vault Meta"
                value={metaVersion ? `v${metaVersion}` : "n/a"}
              />
              <CompactInfoPill
                label="Last Unlock"
                value={formatTimestamp(trust.lastUnlockAt)}
                full
              />
            </View>
          ...
          >
            <View style={themed($compactMetricRow)}>
              ...
              <CompactInfoPill
                ...
                value={formatTimestamp(trust.elevatedUntil)}
              />
            </View>

            <View style={themed($singleActionRow)}>
              <ActionButton
                label="Elevate Session"
            ...
            <Text style={themed($sectionFootnote)}>
              Sensitive actions such as backup changes and recovery require
              step-up.
            </Text>
          </AccordionSection>
    ...
    />
          ...
          >
            {tamperIndicators.length === 0 ? (
              <Text style={themed($securityTileBodyText)}>
                No suspicious recent indicators.
              </Text>
            ) : (
...
function PanelCard({
  ...
  return (
    ...
    >
      ...
      {subtitle ? (
        <Text style={themed($sectionSubtitle)}>{subtitle}</Text>
      ) : null}
      {children}
    </View>
  );
  ...
  return (
    ...
    >
      <Pressable
        onPress={() => setOpen((v) => !v)}
        style={({ pressed }) => [
          themed($accordionHeader),
          pressed && themed($accordionHeaderPressed),
        ]}
      >
        <View style={themed($accordionHeaderLeft)}>
          ...
          <View style={themed($accordionTitleWrap)}>
            ...
            {subtitle ? (
              <Text style={themed($accordionSubtitle)}>{subtitle}</Text>
            ) : null}
          </View>
        </View>
        ...
        <View style={themed($accordionHeaderRight)}>
          {rightValue ? (
            <Text style={themed($accordionRightValue)}>{rightValue}</Text>
          ) : null}
          <Ionicons
  ...
  return (
    ...
    >
      <View style={themed($compactToggleTopRow)}>
        ...
        <View
          style={themed([
            active ? $compactToggleBadgeActive : $compactToggleBadge,
            $compactToggleBadgeBase,
          ])}
        >
          <Text
            style={themed(
              active ? $compactToggleBadgeTextActive : $compactToggleBadgeText,
            )}
          >
            {value}
          </Text>
  ...
  return (
    <View style={themed($securityTileOuter)}>
      ...
      >
        ...
        {icon ? (
          <Ionicons name={icon} size={52} style={themed($securityTileIcon)} />
        ) : null}
        ...
        <View style={themed($securityTileInner)}>
          <View style={themed($securityTileTextWrap)}>
            <Text
              preset="bold"
              style={themed($securityTileTitle)}
              numberOfLines={2}
            >
              {title}
            </Text>
...
function GlassActivityCell({
  ...
  return (
    <View
      style={[
        themed($activityCellV2),
        cell.active ? themed($activityCellActiveV2) : null,
        cell.tone === "warning" ? themed($activityCellWarningV2) : null,
        cell.tone === "critical" ? themed($activityCellCriticalV2) : null,
      ]}
    >
      ...
      <View
        pointerEvents="none"
        style={[styles.cellGlow, { opacity: cell.active ? 0.16 : 0 }]}
      />
...
function buildActivityGrid(events: SecurityAuditEvent[]) {
  ...
  events.forEach((event) => {
    ...
    const bucketIndex =
      ACTIVITY_BUCKETS -
      1 -
      Math.floor(delta / (ACTIVITY_WINDOW_MS / ACTIVITY_BUCKETS));
    if (bucketIndex < 0 || bucketIndex >= ACTIVITY_BUCKETS) return;

    const nextScore =
      event.severity === "critical" ? 6 : event.severity === "warning" ? 4 : 2;
    buckets[bucketIndex].score = Math.min(
      ACTIVITY_ROWS,
      buckets[bucketIndex].score + nextScore / 2,
    );
...
const $deferredPlaceholder: ThemedStyle<ViewStyle> = ({ colors, spacing }) => ({
  minHeight: 140,
  borderRadius: 22,
  padding: spacing.md,
  backgroundColor: "rgba(12, 12, 24, 0.38)",
  borderWidth: 1,
  borderColor: colors.vaultHub.vaultHubBorderSubtle,
});
...
const $actionButtonTextPrimary: ThemedStyle<TextStyle> = ({
  colors,
  typography,
}) => ({
  color: colors.vaultHub.vaultHubTextPrimary,
...
const $actionButtonTextSecondary: ThemedStyle<TextStyle> = ({
  colors,
  typography,
}) => ({
  color: colors.vaultHub.vaultHubTextPrimary,
...
const $actionButtonTextDanger: ThemedStyle<TextStyle> = ({
  colors,
  typography,
}) => ({
  color: colors.error,
...
const $settingPillTextActive: ThemedStyle<TextStyle> = ({
  colors,
  typography,
}) => ({
  color: colors.vaultHub.vaultHubTextPrimary,
...
const $textFieldWrapperCompact: ThemedStyle<ViewStyle> = ({
  colors,
  spacing,
}) => ({
  borderRadius: 16,
...
const $textFieldLabelCompact: ThemedStyle<TextStyle> = ({
  colors,
  typography,
}) => ({
  color: colors.vaultHub.vaultHubTextSecondary,
...
const $summaryStatLabelV2: ThemedStyle<TextStyle> = ({
  colors,
  typography,
}) => ({
...
const $summaryStatValueV2: ThemedStyle<TextStyle> = ({
  colors,
  typography,
}) => ({
