...
function logSettings(message: string, meta?: Record<string, unknown>) {
  if (!__DEV__) return;
  console.log(`[SettingsHome] ${message}`, meta ?? "");
}

const SettingsHomeScreenComponent: FC<
  SettingsStackScreenProps<"SettingsHome">
> = function SettingsHomeScreenComponent(props) {
  ...
  if (__DEV__) {
    ...
    console.log("[Settings] render", renderCountRef.current);
  }

  useEffect(() => {
    logSettings("mounted", { time: nowMs().toFixed(1) });
  }, []);

  useFocusEffect(
    useCallback(() => {
      const focusTime = nowMs();
      if (__DEV__) console.log("[Settings] focus");
      logSettings("focus", { time: focusTime.toFixed(1) });
      return undefined;
    }, []),
  );
          ...
          <View style={themed($grid)}>
            <ControlTile
              ...
              icon={
                <Cloud
                  size={18}
                  color={theme.colors.vaultHub.vaultHubAccentPink}
                />
              }
            ...
            <ControlTile
              ...
              icon={
                <KeyRound
                  size={18}
                  color={theme.colors.vaultHub.vaultHubAccentPinkSoft}
                />
              }
...
export const SettingsHomeScreen = memo(SettingsHomeScreenComponent);
...
function ControlTile({
  ...
  return (
    <Pressable
      onPress={onPress}
      style={({ pressed }) => [themed($tile), pressed && themed($tilePressed)]}
    >
      <View
        style={themed([
          $tileIconWrap,
...
const $settingsPlaceholder: ThemedStyle<ViewStyle> = ({ colors, spacing }) => ({
  minHeight: 220,
  borderRadius: 24,
  padding: spacing.md,
  backgroundColor: "rgba(24, 27, 37, 0.32)",
  borderWidth: 1,
  borderColor: colors.vaultHub.vaultHubBorderSubtle,
});
